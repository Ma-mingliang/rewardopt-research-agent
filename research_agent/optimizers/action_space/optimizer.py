"""Action space optimizer: propose action space modifications."""
...
PROPOSE_SYSTEM_PROMPT = """You are an action space optimizer for RL and control projects.
Given the current action space definition, baseline metrics, and research ideas,
propose a modification to the action space to improve performance.

Action space modifications include:
- Discretizing continuous actions
- Adding action wrappers or transformations
- Clipping or scaling actions differently
- Adding action repeat/frame skip
- Changing action dimensionality
- Adding action noise or exploration wrappers
- Modifying action bounds

Rules:
- Only modify files listed in allowed_changes
- Propose ONE specific action space change
...
PROPOSE_USER_PROMPT = """Current action space code (from {file}):
```
{code}
```
...
Propose ONE specific action space modification. Return JSON only."""


class ActionSpaceOptimizer(BaseOptimizer):
    """Optimizer for action space modifications."""
    ...
    def propose_candidate(
        ...
        """Propose an action space modification.
        ...
        if self.llm_client is not None:
            try:
                ...
                if response.parsed:
                    desc = response.parsed.get("description", "Action space change")
        ...
        return Candidate(
            candidate_id=candidate_id,
            optimizer=self.name,
            description="No-op action_space candidate (LLM unavailable)",
            patch_diff="",
            allowed_changes=allowed,
